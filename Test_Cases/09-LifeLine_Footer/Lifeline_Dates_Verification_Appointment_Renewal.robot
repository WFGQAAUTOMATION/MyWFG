*** Settings ***
Documentation    A test suite to verify MyWFG LifeLine Appointment Renewal Expiration dates
...
...               This test will log into MyWFG and verify that MyWFG LifeLine Appointment Renewal
...
#   ***** Convert Dates difference from sec into min then into hours and into days.
    ...
    ...    log    Appointment Renewal Red notification should be archived
    ...    ELSE IF    ${Notification_TypeID} == 1 and ${Dates_Diff} >=10
    ...    log    Appointment Renewal Red notification was displayed too early
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} < 12
    ...    log    '(Expired)' is missing in expired Appointment Renewal Red notification Due Date
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} > 12
    ...    log    Appointment Renewal Red notification test Passed
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 10
    ...    log    Appointment Renewal Red notification test Passed

    Run Keyword If    ${Notification_TypeID} == 2 and ${Dates_Diff} < 10
    ...    log    Appointment Renewal Yellow notification should be a Red notification
