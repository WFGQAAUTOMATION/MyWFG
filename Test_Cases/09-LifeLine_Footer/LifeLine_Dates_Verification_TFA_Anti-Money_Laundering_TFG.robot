...
#   ***** Convert Dates difference from sec into min then into hours and into days.
    ...
    ...    log    TFA Anti-Money Laundering(TFG) Red notification was displayed too early
    ...    ELSE IF    ${Notification_TypeID} == 1 and ${Dates_Diff} <= 30
    ...    log    TFA Anti-Money Laundering(TFG) Red notification test Passed

    Run Keyword If    ${Notification_TypeID} == 2 and ${Dates_Diff} <= 30
    ...    log    TFA Anti-Money Laundering(TFG) Yellow notification should be a Red notification
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 60
    ...    log    TFA Anti-Money Laundering(TFG) Yellow notification was displayed too early
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 30
    ...    log    TFA Anti-Money Laundering(TFG) Yellow notification test Passed
