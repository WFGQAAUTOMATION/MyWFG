*** Settings ***
Documentation    A test suite to verify MyWFG LifeLine New Rep Training Course Renewal Expiration dates
...
...               This test will log into MyWFG and verify that MyWFG LifeLine New Rep Training Course Renewal
...
#   ***** Convert Dates difference from sec into min then into hours and into days.
    ...
    ...    log    New Rep Training Course Renewal Red notification was displayed too early
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} <= 30
    ...    log    New Rep Training Course Renewal Red notification test Passed

    Run Keyword If    ${Notification_TypeID} == 2 and ${Dates_Diff} <= 30
    ...    log    New Rep Training Course Renewal Yellow notification should be a Red notification
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 60
    ...    log    New Rep Training Course Renewal Yellow notification was displayed too early
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 30
    ...    log    New Rep Training Course Renewal Yellow notification test Passed
