*** Settings ***
Documentation    A test suite to verify MyWFG LifeLine IRS 8233 Form Expiration dates
...
...               This test will log into MyWFG and verify that MyWFG LifeLine IRS 8233 Form notification
...
#   ***** Convert Dates difference from sec into min then into hours and into days.
    ...
    ...    log    IRS 8233 Form Red notification was displayed too early
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} < 12
    ...    log    '(Expired)' is missing in expired IRS 8233 Form Red notification Due Date
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} > 12
    ...    log    IRS 8233 Form Red notification test Passed
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} <= 30
    ...    log    IRS 8233 Form Red notification test Passed

   Run Keyword If    ${Notification_TypeID} == 2 and ${Dates_Diff} <= 30
    ...    log    IRS 8233 Form Yellow notification should be a Red notification
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 60
    ...    log    IRS 8233 Form Yellow notification was displayed too early
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 30
    ...    log    IRS 8233 Form Yellow notification test Passed
