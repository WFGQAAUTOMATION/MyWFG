*** Settings ***
Documentation    A test suite to verify MyWFG LifeLine Long Term Care Renewal Expiration dates
...
...               This test will log into MyWFG and verify that MyWFG LifeLine Long Term Care Renewal
...
Select Agent and Login to MyWFG.com and Check LifeLine
    ...
    ...    ${LL_LTCR_STATE}   ${HOSTNAME}    ${WFG_DATABASE}
...
#   ***** Convert Dates difference from sec into min then into hours and into days.
    ...
    ...    log    Long Term Care Renewal Red notification was displayed too early
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} < 12
    ...    log    '(Expired)' is missing in expired Long Term Care Renewal Red notification Due Date
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} > 12
    ...    log    Long Term Care Renewal Red notification test Passed
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} <= 30
    ...    log    Long Term Care Renewal Red notification test Passed

   Run Keyword If    ${Notification_TypeID} == 2 and ${Dates_Diff} <= 30
    ...    log    Long Term Care Renewal Yellow notification should be a Red notification
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 60
    ...    log    Long Term Care Renewal Yellow notification was displayed too early
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} > 30
    ...    log    Long Term Care Renewal Yellow notification test Passed
