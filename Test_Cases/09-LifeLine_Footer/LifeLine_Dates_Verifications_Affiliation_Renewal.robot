...
#   ***** Convert Dates difference from sec into min then into hours and into days.
    ...
    ...    log    Affiliation Renewal Red notification should be archived
    ...    ELSE IF    ${Notification_TypeID} == 1 and ${Dates_Diff} >=10
    ...    log    Affiliation Renewal Red notification was displayed too early
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} < 12
    ...    log    '(Expired)' is missing in expired Affiliation Renewal Red notification Due Date
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 0 and ${DateDue_Length} > 12
    ...    log    Affiliation Renewal Red notification test Passed
    ...    ELSE IF     ${Notification_TypeID} == 1 and ${Dates_Diff} < 10
    ...    log    Affiliation Renewal Red notification test Passed

    Run Keyword If    ${Notification_TypeID} == 2 and ${Dates_Diff} < 10
    ...    log    Affiliation Renewal Yellow notification should be a Red notification
    ...    ELSE IF    ${Notification_TypeID} == 2 and ${Dates_Diff} >= 10
    ...    log    Affiliation Renewal Yellow notification test Passed
